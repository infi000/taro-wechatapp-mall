 ...
 */
const GoodsDetail = () => {
  const { detail, relatedGoods, buysRecordList, isShowBuysPage } = useSelector((state) => state.goodsShow);
  ...
  useEffect(() => {
    const { params } = router;
    dispatch({ type: 'goodsShow/updateGid', payload: params.gid });
    ...
    <View className='goodsshow-wrap'>
      ...
      <Swiper className='swiper-con' indicatorColor='#999' indicatorActiveColor='#ddd' circular indicatorDots autoplay>
        {detail.colorimgs &&
          ...
          detail.colorimgs[0].map((item) => {
            const { clothcolor, id } = item;
            return (
              <SwiperItem key={id}>
                <View className='swiper-img-con'>
                  <Image mode='heightFix' lazyLoad style='width: 100%;height: 100%' src={clothcolor} />
                </View>
              </SwiperItem>
            );
          })}
      ...
      <View className='goodsshow-con goods-sale-con'>
        <View className='at-row at-row__justify--between goods-sale-top'>
          ...
          <View className='at-col at-col-5 goods-sale-all-btn' onClick={handleShowBuysPage}>全部</View>
        </View>
      ...
      <View className='goodsshow-con goods-warning-con'>
        ...
        {detail.detailimgs &&
          detail.detailimgs.map((item) => {
            const { clothcolor, id } = item;
            return <Image key={id} mode='widthFix' lazyLoad style='width: 100%;height: 100%;float:left' src={clothcolor} />;
          })}
      </View>
      {/* 热门推荐 */}
