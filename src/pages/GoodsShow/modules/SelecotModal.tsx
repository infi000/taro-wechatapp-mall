...
interface IProps {
  ...
  groupprice: string[];
  size: string[];
  handleAdd: (params: { price: string; color: string; size: string; num: number }) => void;
  handleCancel: () => void;
}
const SelecotModal = (props: IProps) => {
  ...
  const handleSubmit = () => {
    const { price, color, size, num } = formParams;
    if (isEmpty(price) || isEmpty(color) || isEmpty(size) || num <= 0) {
      showToast('选择颜色和尺码');
      return;
    }
    handleAdd({ price, color, size, num });
    reset();
  };
      ...
      <View className='selecotCon'>
        ...
        <View className='selecotItem at-row at-row--wrap'>
          ...
          <View className='at-col at-col-10'>
            {formParams.color || '-'} / {formParams.size || '-'}
          </View>
        </View>
        <View className='selecotItem at-row at-row--wrap'>
        ...
        <View className='selecotItem at-row at-row--wrap'>
          <View className='at-col at-col-2 selecotTag'> 尺码：</View>
          <View className='at-col at-col-10'>
